...
properties:addItem("Color scheme").onTouch = function()
  ...
  local startX, startY =
  math.floor(container.width / 2 - ((pickerWidth + separatorWidth) * #config.colorScheme) / 2) + separatorWidth,
      math.floor(container.height / 2 - 3)
  container:addChild(GUI.label(1, startY, container.width, 1, 0xFFFFFF, "Color scheme")):setAlignment(
  GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP)
  startY = startY + 3

  for i = 1, #config.colorScheme do
    if i == 1 then
      container:addChild(GUI.slider(startX, startY + 4, pickerWidth, 0x66DB80, 0x1e1e1e, 0xFFFFFF, 0xAAAAAA, 0, 100, config.backgroundTransparency * 100, false, "Transparency ", " %")).onValueChanged = function(
        workspace, slider)
    ...
    elseif i == 3 then
      container:addChild(GUI.slider(startX, startY + 4, pickerWidth, 0x66DB80, 0x1e1e1e, 0xFFFFFF, 0xAAAAAA, 0, 100, config.titleTransparency * 100, false, "Transparency ", " %")).onValueChanged = function(
        workspace, slider)
    ...
    local colorSelector = container:addChild(GUI.colorSelector(startX, startY, pickerWidth, separatorWidth,
      config.colorScheme[i][2], config.colorScheme[i][1]))
...
properties:addItem("Terminal").onTouch = function()
  ...
  container:addChild(GUI.label(1, startY, container.width, 1, 0xFFFFFF, "Terminal")):setAlignment(
  GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP)
  startY = startY + 3

  local titleBar = container:addChild(GUI.switchAndLabel(startX, startY, 29, 8, 0x66DB80, 0x2D2D2D, 0xE1E1E1, 0x878787,
    "Title bar:", config.title))
  ...
  local shadow = container:addChild(GUI.switchAndLabel(startX, startY, 29, 8, 0x66DB80, 0x2D2D2D, 0xE1E1E1, 0x878787,
  ...
  local width = container:addChild(GUI.input(startX + 13, startY, 6, 1, 0xEEEEEE, 0x555555, 0x999999, 0xFFFFFF, 0x2D2D2D,
    window.gpu.temp.width, nil, true))
  ...
  local height = container:addChild(GUI.input(startX + 22, startY, 6, 1, 0xEEEEEE, 0x555555, 0x999999, 0xFFFFFF, 0x2D2D2D,
    window.gpu.temp.height, nil, true))
...
container:attachComponent(components.createFilesystem(componentAddresses.filesystem, filesystem.get(currentScriptPath),
  currentScriptPath .. "rootfs/"))
container:attachComponent(components.createFilesystem(componentAddresses.tmpfs, component.proxy(computer.tmpAddress()),
  "/OpenOS/"))
window.gpu = container:attachComponent(components.createGPU(componentAddresses.gpu, config.colorScheme[1][2],
  config.colorScheme[2][2], function() return windowsContainer.width end,
  function() return windowsContainer.height - (window.titleBar.hidden and 0 or 1) end,
  container.libcomponent.list("screen")()))
...
window.gpu.context.draw = function()
  ...
  local bufferIndex, indexStepOnReachOfSquareWidth, gpuIndex =
  screen.getIndex(window.gpu.context.x, window.gpu.context.y), bufferWidth - window.gpu.temp.width, 1
      ...
      for x = 1, window.gpu.temp.width do
        if (window.gpu.context.x + x - 1) >= workspace.x and (window.gpu.context.x + x - 1) <= workspace.x + workspace.width - 1 then
          if config.backgroundTransparency > 0 and window.transparencyEnabled then
            ...
            newFrameSymbols[bufferIndex] = emptySymbol and
            window.gpu.temp.backgrounds[gpuIndex] == config.colorScheme[1][2] and newFrameSymbols[bufferIndex] or
            window.gpu.temp.symbols[gpuIndex]
            newFrameBackgrounds[bufferIndex] = window.gpu.temp.backgrounds[gpuIndex] == config.colorScheme[1][2] and
            color.blend(newFrameBackgrounds[bufferIndex], config.colorScheme[1][2], config.backgroundTransparency) or
            window.gpu.temp.backgrounds[gpuIndex]
            newFrameForegrounds[bufferIndex] = emptySymbol and
            color.blend(newFrameForegrounds[bufferIndex], config.colorScheme[1][2], config.backgroundTransparency) or
            window.gpu.temp.foregrounds[gpuIndex]
          else
