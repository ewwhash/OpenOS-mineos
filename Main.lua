...
local function bootstrap()
    ...
    local result, reason = download("https://github.com/ewwhash/OpenOS-mineos/blob/master/rootfs.pkg?raw=true", currentScriptPath .. "Temp.pkg", function(progress)
...
window.titleBar.maximize.onTouch = function()
    window:maximize()
    window.gpu:flush(window.width, window.height - 1, true, nil, nil, config.colorScheme[1][2], config.colorScheme[2][2])
end
